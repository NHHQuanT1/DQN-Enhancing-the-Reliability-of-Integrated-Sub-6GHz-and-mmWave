...
class QNetworkManager:
    ...
    def update_q_network(self, network_idx, state, action, reward, next_state): #network_idx giá trị này chính là giá trị sinh ra ở Poisson
        """
        Cập nhật Q-network theo công thức 23:
        Q(s,a) = Q(s,a) + α(s,a) * [u(r + γ*max_a'Q(s',a') - Q(s,a)) - x_0]
        """
        # 1. Cập nhật V và alpha
        self.update_v_alpha(network_idx, state, action) #network_idx giá trị này chính là giá trị sinh ra ở Poisson
        
        # 2. Lấy network và optimizer tương ứng được sinh ra ở pp Poisson
        network = self.q_networks[network_idx]
        optimizer = self.optimizers[network_idx]
        
        # 3. Chuyển state thành tensor
        state_tensor = torch.FloatTensor(state.flatten()).unsqueeze(0) #làm phẳng, chuyển thành Torch tensor và thêm 1 chiều để phù hợp đầu vào network
        next_state_tensor = torch.FloatTensor(next_state.flatten()).unsqueeze(0)
        
        # 4. Tính Q-values hiện tại
        q_values = network(state_tensor) #lấy tất cả giá trị cho tất cả hành động
        action_idx = action_to_index(action)
        ...
        # 8. Lấy alpha từ bảng
        key = state_action_to_key(state, action) #chuyển key thành 1 tuple duy nhất
        alpha_value = self.alpha[network_idx].get(key, 1.0) #chưa có trả về 1, nếu key có rồi thì thực hiện tính toán anpha
